...
external_flags = [
    ...
    '-isystem',
    '$PONYAI_PATH/.sub-repos/make8-bin/external',
    '-isystem',
    '$PONYAI_PATH/.sub-repos/make8-bin/external/glog/',
    '-isystem',
    '$PONYAI_PATH/.sub-repos/make8-bin/external/pstreams/',
    '-isystem',
...
def bazel_info():
  ...
  p = subprocess.Popen(['bazel', 'info'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def create_missing_packages(err):
  ...
  for package in missing_packages:
    if package not in created_packages:
      created_packages.add(package)
      if not os.path.exists(package):
        os.makedirs(package)
      with open(package + '/BUILD', 'w+') as f:
...
def bazel_query(args):
  ...
  while True:
    p = subprocess.Popen(query_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    output, err = p.communicate()
...
def FlagsForFile(filename, **kwargs):
  ...
  p = subprocess.Popen(bazel_aspects, stdout = subprocess.PIPE, stderr = subprocess.PIPE)
  _, err = p.communicate()
  if err:
    print >> sys.stderr, err
